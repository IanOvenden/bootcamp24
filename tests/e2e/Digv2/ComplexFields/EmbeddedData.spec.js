...
test.describe('E2E test', () => {
  test('should login, create case and run different test cases for Embedded Data', async ({
    ...
    expect(
      attributes.includes('readonly') && (await street.inputValue()) === 'Main St'
    ).toBeTruthy();

    const city = page.locator('input[data-test-id="57d056ed0984166336b7879c2af3657f"]');
    attributes = await common.getAttributes(city);
    expect(
      attributes.includes('readonly') && (await city.inputValue()) === 'Cambridge'
    ).toBeTruthy();
    ...
    expect(
      attributes.includes('readonly') && (await postalCode.inputValue()) === '02142'
    ).toBeTruthy();
