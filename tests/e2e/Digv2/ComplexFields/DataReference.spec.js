...
test.describe('E2E test', () => {
  test('should login, create case and run different test cases for Data Reference', async ({
    ...
    const basicProductRecord = await assignment.locator(
      'tr:has-text("Basic Product") >> input[type="radio"]'
    );
    ...
    const luxuryProductRecord = await assignment.locator(
      'tr:has-text("Luxury Product") >> input[type="checkbox"]'
    );
    attributes = await common.getAttributes(luxuryProductRecord);
    expect(attributes.includes('checked')).toBeTruthy();

    const greenItemRecord = await assignment.locator(
      'tr:has-text("Green Item") >> input[type="checkbox"]'
    );
