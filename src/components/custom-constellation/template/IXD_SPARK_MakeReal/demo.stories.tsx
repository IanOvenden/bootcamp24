...
import IxdSparkMakeReal from './index';
...
const meta: Meta<typeof IxdSparkMakeReal> = {
  title: 'IxdSparkMakeReal',
  component: IxdSparkMakeReal,
  excludeStories: /.*Data$/,
  parameters: {
    type: 'Form'
  }
...
export default meta;
type Story = StoryObj<typeof IxdSparkMakeReal>;
...
export const BaseIxdSparkMakeReal: Story = args => {
  const props = {
    getPConnect: () => {
      return {
        getChildren: () => {
  ...
  return (
    <>
      <IxdSparkMakeReal {...props} {...args}>{regionAChildren}</IxdSparkMakeReal>
    </>
  );
};

BaseIxdSparkMakeReal.args = {
  showLabel: true,
  label: 'Form template',
  NumCols: 1,
};
